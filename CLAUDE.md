...
- Extraction cadence: `Extractor` picks its source per var_key. A daily store answers for itself; an **hourly** store (`atm-instante`, `atm-accum-avg`, `radiation`, `waves`) answers only sub-daily input, and a date-only query is served from **h2ds** — so extracting those at daily cadence needs a current `compile`. Sub-daily input broadcasts the daily-by-construction features (ekman chain, `wind_*`) across each day. Units differ between the two sources (`msl` is Pa hourly, hPa in h2ds). See `docs/api/extractor.md#cadence`.
